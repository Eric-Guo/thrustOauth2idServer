...
CGO_ENABLED ?= 1
USE_DOCKER ?= 1
DEFAULT_DOCKER_IMAGE := swr.cn-north-4.myhuaweicloud.com/ddn-k8s/docker.io/golang:1.27.1-bookworm
DEFAULT_DOCKER_IMAGE_ARM64 := docker.io/library/golang:1.27.1-bookworm
...
graph:
	...
	@cp -f cmd/thrustOauth2idServer/main.go .
	go-callvis -skipbrowser -format=svg -nostd -file=thrustOauth2idServer thrust_oauth2id
	@rm -f main.go thrustOauth2idServer.gv
...
clean:
	@rm -vrf cmd/thrustOauth2idServer/thrustOauth2idServer*
	@rm -vrf cover.out
	@rm -vrf main.go thrustOauth2idServer.gv
	@rm -vrf internal/ecode/*.go.gen*
	@rm -vrf internal/routers/*.go.gen*
	@rm -vrf internal/handler/*.go.gen*
	@rm -vrf internal/service/*.go.gen*
	@rm -rf thrustOauth2idServer-binary.tar.gz
	@echo "clean finished"
