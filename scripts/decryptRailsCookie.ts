...
// Works with Rails 8.0.3, 7.2.2.2, 7.1.5.2
...
async function decryptCookie(encryptedCookie: string, cookieName: string, secretKeyBase: string) {
  ...
  const hashDigest = 'sha256'; // sha256 for Rails 8.0.3, 7.2.2.2, sha1 for Rails 7.1.5.2
...
// Run script as node --experimental-transform-types decryptRailsCookie.ts
// https://www.urldecoder.org need decode cookie first if copy from browser
...
// get from tmp/local_secret.txt if you are in development
